...
IE盒子模型的盒子高度：左右的border+左右的padding+width。

在CSS3中引入了box-sizing属性，box-sizing：content-box 表示标准盒子模型。box-sizing：border-sizing 表示的是IE盒子模型
...
个人理解：内联样式(标签内选择符)>id选择器>类选择器>元素选择器 !important优先级最高
```
...
```
设备像素指的是物理像素，一般手机的分辨力就是设备像素，一个设备的设备像素是不可变的。
